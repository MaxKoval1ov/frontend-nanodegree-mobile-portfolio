...
const pizzasContainers = []
const pizzasDiv = document.getElementById("randomPizzas");


  for (var i = 0; i < 100; i++) {
    const newElem = pizzaElementGenerator(i);
    pizzasContainers.push(newElem);
    pizzasDiv.appendChild(newElem);
  }
...
function changeSliderLabel(size) {
  switch(size) {
    case "1":
      pizzaSize.innerText = "Small";
      return;
    case "2":
      pizzaSize.innerText = "Medium";
      return;
    case "3":
      pizzaSize.innerText = "Large";
      return;
    default:
...
const randomPizzas = document.querySelector("#randomPizzas");
...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    for (var i = 0; i < pizzasContainers.length; i++) {
      var dx = determineDx(pizzasContainers[i], size);
      var newwidth = `${pizzasContainers[i].offsetWidth + dx}px`;
...
window.addEventListener('scroll', debounce(updatePositions,15));
